...
//------------------------------------------------------------------------------
#define pr_err(...)		LogWrite (FromBcm54213, CIRCLE_LOG_ERROR, ##__VA_ARGS__)
#define pr_warn(...)	LogWrite (FromBcm54213, CIRCLE_LOG_WARNING, ##__VA_ARGS__)
#define pr_info(...)	LogWrite (FromBcm54213, CIRCLE_LOG_NOTICE, ##__VA_ARGS__)
#define pr_debug(...)	LogWrite (FromBcm54213, CIRCLE_LOG_DEBUG, ##__VA_ARGS__)

#define mdelay(ms)		MsDelay (ms)
#define udelay(us)		usDelay (us)
#define ndelay(ns)		nsDelay (ns)

#define CLOCK_FREQ_HZ	1000000 // system timer runs at 1MHz
...
boolean CBcm54213Device::Initialize (void)
{
	...
	if (major != GENET_V5)
	{
		pr_err("GENET version mismatch, got: %d, configured for: %d",
...
void CBcm54213Device::mdio_wait(void)
{
	...
	do
	{
        // wait at least 1000000/100 = 10000 ticks -> 10000 usecs
		if ( (GetClockTicks ()-nStartTicks) >= (CLOCK_FREQ_HZ / HZ) )
		{
			break;
		}
	}
