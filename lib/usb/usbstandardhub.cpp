...
boolean CUSBStandardHub::Initialize (void)
{
	...
	{
        LogWrite(FromHub, CIRCLE_LOG_ERROR, "Cannot get hub descriptor");
...
void CUSBStandardHub::HandlePortStatusChange (void)
{
	...
	if (usStatusBitmap & 1)
	{
        LogWrite(FromHub, CIRCLE_LOG_PANIC, "Hub status change not handled");
	}
	...
	for (unsigned nPort = 0; nPort < m_nPorts; nPort++)
	{
		...
		if (GetHost ()->ControlMessage (GetEndpoint0 (),
			...
			GET_STATUS, 0, nPort+1, m_pStatus[nPort], 4) != 4)
		{
            LogWrite(FromHub, CIRCLE_LOG_PANIC,
